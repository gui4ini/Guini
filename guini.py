...
if __name__ == "__main__":
    ...
    try:
        ...
        target_script_path = script_dir / "gui4ini_v0.7.py"
        ...
        if not target_script_path.exists():
            ...
            error_message = (
                f"Error: The main application file 'gui4ini_v_0.7.py' was not found in the directory:\n"
                f"{script_dir}"
            )
        ...
        print(f"The application 'gui4ini_v0.7.py' exited with an error (code {e.returncode}).", file=sys.stderr)
        sys.exit(e.returncode)
    except Exception as e:
        # Catch any other unexpected errors during launch
        print(f"An unexpected error occurred while trying to launch 'gui4ini_v0.7.py':\n{e}", file=sys.stderr)
        sys.exit(1)
